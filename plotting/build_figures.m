function build_figures(options)
...
function build_error_boxplots_together(datasets, methods, dataset_labels, ymaxs, dpi, options)
...
if (options.DNApoly == false)
    ...
    print('-dtiff', dpi, 'images/error_boxplots_all_DNApoly.tiff');
end
close
end
...
data = load_results_for_condition('ishii', 'sim_secr', condition, methods, options);
data(abs(data) < TOL) = 0;
...
if (options.DNApoly == false)
    ...
    filename = sprintf('images/secretion_%s_DNApoly.tiff', 'ishii');
end
...
[~, reaction_idx_data] = ismember(reaction_ids, expdata.reactions);
reference = expdata.fluxomics(reaction_idx_data, cond_idx);
data = load_results_for_condition(dataset, 'sim_secr', condition, methods, options);
data(abs(data) < TOL) = 0;
...
if (options.DNApoly == false)
    ...
    filename = sprintf('images/secretion_%s_DNApoly.tiff', dataset);
end
print('-dtiff', dpi, filename);
close
...
for i = 1:length(methods)
    ...
    filename_DNApoly = sprintf('results/%s_%s_%s_DNApoly.mat', methods{i}, dataset, experiment_type);
    if (options.DNApoly == true)
        if exist(filename_DNApoly, 'file')
            filename = filename_DNApoly;
        else
            fprintf('%s does not exist, replacing with %s\n', filename_DNApoly, filename);
        end
    end
    load(filename);
